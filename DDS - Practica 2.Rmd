...
```{r histograma}
# Me quedo con los enlaces únicos

links_unicos <- table(df_links$links)

df_unicos <- data.frame(links_unicos,)

# Crear una nueva columna para indicar si el enlace es absoluto o relativo
links_unicos$tipo_enlace <- ifelse(grepl("^http", df_links$links) | grepl("^//", df_links$links), "Absoluto", "Relativo")
