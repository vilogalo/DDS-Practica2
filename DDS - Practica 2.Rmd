...
Sabemos que las URLs relativas ya apuntan dentro, entonces solo quedaría añadirles las URLs absolutas que empiecen por "https://www.mediawiki.org". Hay que tener en cuenta que los links "//commons.wikimedia.org/wiki/Special:UploadWizard" y "//techblog.wikimedia.org/" son absolutos y no apuntan al dominio base, mientras que "//m.mediawiki.org/w/index.php?title=MediaWiki&mobileaction=toggle_view_mobile" si que apunta al dominio base. Una vez tratadas estas excepciones, vamos a clasificar el dataframe de enlaces totales (sin buscar solo los únicos) en "Externos" e "Internos" y pintar el gráfico de barras. 

```{r gráfico barras}

df_links$ExtInt <- ifelse(grepl("mediawiki.org", df_links$links) | grepl("^/", df_links$links) | grepl("^#", df_links$links), "Interno", "Externo")

# Ahora excluyo las excepciones:

#https://stats.wikimedia.org/#/www.mediawiki.org
#//techblog.wikimedia.org/
#//commons.wikimedia.org/wiki/Special:UploadWizard



frec_extint <- table(df_links$ExtInt)
df_frec_extint <- as.data.frame(frec_extint)

grafico_barras_extint <- barplot(df_frec_extint$Freq,
                   col = c("red","blue"),
                   xlab = "Enlaces",
                   main = "Frecuencia de enlaces externos e internos",
                   legend.text = c("Externos", "Internos"))
